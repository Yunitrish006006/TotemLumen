...
/** One compute command buffer inserted into Minecraft's current graphics submission. */
public final class VulkanFrameComputeBatch {
    ...
    public static VulkanFrameComputeBatch begin(
        ...
        if (!capabilities.canUseMinecraftFrameSubmissionForCompute()) {
            throw new IllegalStateException("Minecraft graphics queue does not advertise VK_QUEUE_COMPUTE_BIT");
        }
    ...
    public void finishAndEnqueue(Runnable afterGpuCompletion) {
        if (finished) {
            throw new IllegalStateException("Compute batch is already finished");
        }
        ...
        commandPool.recycleAfterFence(commandBuffer, afterGpuCompletion);
        finished = true;
    }
}
