...
/**
 * P5 production-shaped debug renderer.
 *
 * <p>Resources are persistent. Camera/header data is uploaded every submitted frame, while voxel
 * data is uploaded only when the selected section set or any section revision changes. Only one
 * frame may be in flight so the mapped upload buffer can never be overwritten while the GPU is
 * reading it. The renderer stays on Minecraft's existing Vulkan device and submission timeline.</p>
 */
public final class P5LiveDebugRenderer {
    ...
    private static final int SECTION_TABLE_WORDS = MAX_SECTIONS * SECTION_META_WORDS;
    private static final int VOXEL_BASE_WORD = HEADER_WORDS + SECTION_TABLE_WORDS;
    ...
    public static void runOnRenderThread() {
        ...
        if (sceneUploadRequired) {
            packSections(upload, sections);
            uploadBytes = (VOXEL_BASE_WORD + sections.size() * SectionVoxelData.VOXEL_COUNT) * Integer.BYTES;
        } else {
        ...
        try {
            ...
            TotemLumenClient.LOGGER.error("P5 live debug frame submission failed", failure);
        }
    }
    ...
    private static void onFrameComplete(boolean fullSceneUpload) {
        ...
        if (!firstFrameLogged) {
            ...
            TotemLumenClient.LOGGER.info(
                    "P5 live debug renderer READY: persistent {}x{} Vulkan DDA target, mode={}, sceneUpload={} ",
    ...
    private static void packHeader(
        ...
        putWord(buffer, 3, Resources.pixelBaseWord(width, height));
    ...
    private static void packSections(ByteBuffer buffer, List<SectionSnapshot> sections) {
        for (int sectionIndex = 0; sectionIndex < sections.size(); sectionIndex++) {
            SectionSnapshot snapshot = sections.get(sectionIndex);
            int meta = HEADER_WORDS + sectionIndex * SECTION_META_WORDS;
            int voxelOffset = VOXEL_BASE_WORD + sectionIndex * SectionVoxelData.VOXEL_COUNT;
    ...
    private static void closeQuietly(AutoCloseable closeable) {
        ...
        try {
            ...
            TotemLumenClient.LOGGER.warn("Failed to close P5 live-debug resource", exception);
        }
    }
    ...
    private static final class Resources {
        ...
        static int pixelBaseWord(int width, int height) {
        ...
        static Resources create(VulkanDevice device, int width, int height) {
            int pixelBaseWord = pixelBaseWord(width, height);
            ...
            try {
                ...
                program = VulkanComputeProgram.create(device, "totem_lumen_p5_live_debug.comp", SHADER, scene);
                commandPool = new VulkanFrameCommandPool(device);
                texture = RenderSystem.getDevice().createTexture(
                        "Totem Lumen P5 live DDA debug target",
                ...
                if (!(texture instanceof VulkanGpuTexture vulkanTexture)) {
                    throw new IllegalStateException("P5 live debug target is not backed by VulkanGpuTexture");
                }
