 ...
 * radiance. It also uses two bounce samples per frame, a longer GI history and a conservative
...
final class P12GiShaderPatch {
    ...
    static final float HISTORY_WEIGHT = 0.95f;
    ...
    static String apply(String source) {
        ...
        String helpers = """
                ...
                uint giTemporalIndirectColor(
                    ...
                    return packRgba(mix(currentRgb, historyRgb, 0.95), 255u);
                }
        ...
        String newMainBranch = """
                if (mode == 10u || mode == 11u) {
                    if (primaryHit.hit == 0u) {
                        ...
                        writeHistory(pixel, width, primaryHit, packRgba(vec3(0.0), 255u));
                    } else {
                        uint indirectColor = giTemporalIndirectColor(
                            primaryHit, origin, direction, pixel, width, height
                        );
                        writeHistory(pixel, width, primaryHit, indirectColor);
                        if (mode == 10u) {
