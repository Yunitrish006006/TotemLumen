...
Alpha 60 Minecraft RGB loading follow-up: non-emissive chunk sections are skipped by palette before 4,096-voxel RGB source scans; sky-colour/ambient values are snapshotted once per client tick for terrain workers; sky-triggered remeshes are capped at 12 sections per tick and yield to block/source updates. Build-time queue-order/budget checks pass. In-world hitch and sky-transition acceptance remains pending.

Further Alpha 60 profiling found that the Minecraft RGB profile still ran Totem-only entity/block-entity/fluid geometry capture and per-frame voxel scene extraction. An idle-client JFR sample attributed 97.6% of sampled Java allocation pressure to dynamic-entity snapshot construction. These capture/extraction paths now run only in the Totem render profile; client chunk/block tracking remains available for switching profiles. The effect on new-chunk hitches needs in-world comparison against Minecraft Pure.

Alpha 60 RGB chunk-flight JFR follow-up: a 35-second in-world capture showed repeated whole-source-map filtering in `RebuildTask` as the largest sampled CPU cost; RGB section reads also dominated sampled Java allocations. Rebuilds now use a section-local source index, and terrain-worker reads reuse a revision-checked last-section lookup. `verifyRgbHotPaths` covers negative coordinates, changed/removed sources, newly populated sections, and unload invalidation; the full local build passes. In-world hitch reduction remains to be measured. Sky palette changes queued roughly 1,200–2,000 terrain sections in the observed 16-chunk view-distance world, but the player was manually switching between day and night during testing; those counts alone are not evidence that ordinary chunk flight is dominated by sky remeshing.

First-stage in-world feedback reports less hitching and normal RGB/geometry. A second 35-second JFR no longer lists the whole-source-map scan among hot methods; RGB lookup allocation pressure dropped from roughly 72% of sampled allocations before the index/cache change to roughly 4% afterward (different flight/world-generation workloads, so these are directional, not a controlled speedup ratio). The follow-up sample identified repeated `ThreadLocal` access while sampling each terrain quad, so one sampler is now reused across all samples of that quad. The final build/check passes and in-world feedback reports similar hitching to the first-stage build, with RGB and geometry still normal. In a third 35-second JFR without manual time switching, `ThreadLocal` access was 1.39% of hot-method samples, RGB section lookup was 1.56% of sampled allocation pressure, and 311 of 312 `PalettedContainer.get` execution samples had no Totem Lumen frame in their stacks (mainly vanilla server ticking, terrain generation, or Indigo chunk meshing). These are workload-specific sample shares, not controlled frame-time speedup measurements. Natural sky epochs still occur, but the present data does not justify another sky-remesh algorithm change; compare against Minecraft Pure under matched new-chunk flight before attributing remaining hitches to RGB.

Minecraft RGB handheld-light visual parity follow-up: a reported white veil, mismatched brightness and shorter-looking falloff were traced to an additive depth-only screen pass while placed RGB uses linear voxel attenuation and texture-multiplied terrain vertex colour. The handheld RGB pass now snapshots the pre-held-light scene and samples a bounded atlas of the same client RGB field used by terrain. It adds only the positive per-channel difference between moving and placed light, with reflectance/headroom weighting and the shared 1.20 radial distance scale. Pure retains its independent blend but uses the same radial distance basis. Totem held lights use the same radial scale in the Vulkan path. This reduces double-brightness when moving and placed emitters overlap without remeshing moving lights. It does not add block occlusion to the depth-only path and requires in-world comparison against a placed source.

Held emitter surface follow-up: luminous `BlockItem` models and lava buckets now carry an emissive marker into Minecraft's item render state and use full-bright lightmap coordinates in first-person, third-person, and dropped-item contexts. This brightens the item surface itself without drawing a separate outline or changing non-luminous items; in-world texture/color parity remains pending visual acceptance.

Minecraft RGB placed-light shape follow-up: client visual propagation now uses 18 face/face-diagonal directions, exact source-to-cell radial distance, and a 1.20 visual falloff scale. Diagonal intermediate blocks still gate propagation. The server-authoritative gameplay field now uses the same shared radial attenuation rule and obstruction penalty, while retaining its bounded rebuild budget and non-rendering server boundary. Dedicated-server spawn behavior and client/server cross-region parity remain pending in-world acceptance.

Historical Alpha 59 validated/runtime baseline (Minecraft 26.2):
